...
public partial class PlayerMovement
{
  private void GroundCheck(in float2 position)
  {
    ...
    _down_raycastHit = Physics2D.BoxCast(
      _down_boxCastOrigin, _down_boxCastSize, 0.0f, Vector2.down, 5.0f
    );
  ...
  private void ObstacleCheck(in float2 position)
  {
    ...
    _right_raycastHit = Physics2D.BoxCast(
      _right_boxCastOrigin, _right_boxCastSize, 0.0f, Vector2.right, 5.0f
    );
    ...
    if (_isObstructed) _dead = CollisionIsLayer(collider.gameObject.layer, in _killableLayer);
  }
